 ...
 *
 * Signature images are fetched from Supabase (stored in admin_signatures),
 ...
 *  - All injectors now use the full Profile (age, sex, civilStatus) instead of
 *    hardcoded fallbacks.
 *  - injectBarangayClearance: sex, civilStatus, and purok are taken from the
 *    user profile / request and properly substituted into the certificate body.
 *  - injectBusinessClearance: owner name and date values resolved from profile.
 *  - injectCertificationOfDeath: requestor name from profile, all deceased
 *    fields from RequestDetail.
 *  - injectJobSeekerCert: name, BCN no., residency years, and date from profile
 *    and request.
 *  - injectOathOfUndertaking: name, age, purok, residency years, and date from
 *    profile and request.
 *  - normaliseProfile now also maps age from raw record.
 */
...
async function injectBarangayClearance(
  zip:     any,
  req:     RequestDetail,
  profile: Profile,
): Promise<void> {
  ...
  await patchXml(zip, 'word/document.xml', xml => {
    const certifyText =
      `${xmlEscape(name)}, ${xmlEscape(age)} years old, ` +
      `${xmlEscape(sex)}, ${xmlEscape(civilStatus)}, Filipino citizen, ` +
      `whose name and signature/right thumb mark appears below is a BONAFIDE and permanent ` +
      `resident of ${xmlEscape(purok)}, BRGY. GUIN-ON, Calbayog City,`;
...
async function injectBusinessClearance(
  zip:     any,
  req:     RequestDetail,
  profile: Profile,
): Promise<void> {
...
async function injectCertificationOfDeath(
  zip:     any,
  req:     RequestDetail,
  profile: Profile,
): Promise<void> {
...
async function injectJobSeekerCert(
  zip:     any,
  req:     RequestDetail,
  profile: Profile,
): Promise<void> {
  ...
  await patchXml(zip, 'word/document.xml', xml =>
    xml
      ...
      // BCN No. — handles both "BCN NO.: 09" inline variant and bare "09"
...
async function injectOathOfUndertaking(
  zip:     any,
  req:     RequestDetail,
  profile: Profile,
): Promise<void> {
  ...
  await patchXml(zip, 'word/document.xml', xml =>
    xml
      ...
      // Age — the template may reference the applicant's age near the residency clause
      .replace(/\b\d+ years old\b/g, `${xmlEscape(age)} years old`)
      // Residency years
      ...
      // Day + ordinal suffix (template uses separate runs for day and suffix)
      .replace(/(<w:t[^>]*>)\d{1,2}(<\/w:t>)(?=[\s\S]{0,200}Barangay Guin-on)/,
               `$1${xmlEscape(day)}$2`)
      .replace(/(<w:t[^>]*>)(ST|ND|RD|TH)(<\/w:t>)(?=[\s\S]{0,200}Barangay Guin-on)/,
               `$1${xmlEscape(suffix.toUpperCase())}$3`),
  );
}
...
// ─── Signature injector (by alt-text, not placeholder tag) ────────────────────
...
async function injectSignatureIntoZip(
  ...
  if (!docFile) return;
  let xml: string = await docFile.async('string');
  let mutableDataUrl = base64DataUrl;
  ...
  const existingMedia = relMatch[1];
  ...
  if (rIdUsageCount <= 1) {
    ...
    const patched     = fromDescr.replace(`r:embed="${existingRId}"`, `r:embed="${newRId}"`);
...
async function generateDigitalSealDataUrl(
  ...
  for (let i = 0; i < topText.length; i++) {
    const angle = topStartAngle + (topArcSpan / (topText.length - 1)) * i;
    ctx.save();
    ctx.translate(CX + topRadius * Math.cos(angle), CY + topRadius * Math.sin(angle));
  ...
  for (let i = 0; i < botText.length; i++) {
    const angle = botStartAngle + (botArcSpan / (botText.length - 1)) * i;
    ctx.save();
    ctx.translate(CX + botRadius * Math.cos(angle), CY + botRadius * Math.sin(angle));
    ctx.rotate(angle - Math.PI / 2);
  ...
  ctx.fillStyle = GOLD;
  const starCX = CX, starCY = CY - 48, outerR = 9, innerR = 4, points = 8;
  ctx.beginPath();
