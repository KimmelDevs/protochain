    ...
    <div className="min-h-screen p-4 lg:p-8">
      <div className="max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">Pending Requests</h1>
          <p className="text-gray-400">Review and process document requests from residents</p>
        </motion.div>
        ...
        >
          {[
            ...
            { label: 'Waiting 2+ Days', value: requests.filter(r => daysWaiting(r.created_at) >= 2).length, color: 'text-red-400' },
            { label: 'Received Today', value: requests.filter(r => daysWaiting(r.created_at) === 0).length, color: 'text-blue-400' },
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          <Card>
            <CardHeader><CardTitle>Pending Requests ({filtered.length})</CardTitle></CardHeader>
            <CardContent>
              {requests.length === 0 ? (
                          ...
                          <TableRow key={req.id}>
                            ...
                            <TableCell>
                              {days === 0 ? (
                                ...
                                <span className={`text-sm flex items-center gap-1 ${days >= 2 ? 'text-red-400' : 'text-yellow-400'}`}>
