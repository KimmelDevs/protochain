...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return <span className={`mono text-[10px] font-bold tracking-[0.12em] uppercase px-2.5 py-1 border ${cfg[status] ?? cfg.pending}`}>{status}</span>;
};
const AlertBanner = ({ variant, children, onClose }: { variant: 'error' | 'success' | 'warning'; children: React.ReactNode; onClose?: () => void }) => {
  const cfg = { error: 'border-red-400 bg-red-50 dark:bg-red-950/30 text-red-700 dark:text-red-400', success: 'border-emerald-400 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400', warning: 'border-amber-400 bg-amber-50 dark:bg-amber-950/30 text-amber-700 dark:text-amber-400' };
  return (
...
const ActionBtn = ({ label, icon: Icon, onClick, disabled, variant = 'default', loading }: { label: string; icon: React.ElementType; onClick: () => void; disabled?: boolean; variant?: 'default' | 'danger' | 'orange'; loading?: boolean }) => {
  const cls = { default: 'bg-[#1a1917] dark:bg-[#f0eee8] text-white dark:text-[#1a1917] hover:bg-[#3d3b36] dark:hover:bg-white border-[#1a1917] dark:border-[#f0eee8]', orange: 'bg-orange-600 text-white hover:bg-orange-700 border-orange-600', danger: 'bg-transparent text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 border-red-400 dark:border-red-700' }[variant];
  return (
...
const HashDisplay = ({ hash, txHash }: { hash: string; txHash?: string | null }) => {
  ...
  return (
    <div className="border-l-2 border-emerald-500 pl-3 py-1 space-y-2">
      ...
      {txHash && (
        <div>
          <div className="flex items-center gap-2 mb-1"><ShieldCheck className="w-3.5 h-3.5 text-blue-500" /><span className="mono text-[10px] font-bold tracking-[0.1em] uppercase text-blue-500">On-Chain (Sepolia)</span></div>
          <a href={`https://sepolia.etherscan.io/tx/${txHash}`} target="_blank" rel="noopener noreferrer" className="mono text-[10px] text-blue-500 hover:underline break-all">{txHash.slice(0, 20)}…{txHash.slice(-10)} ↗</a>
        </div>
      )}
    </div>
  );
...
export default function ReviewRequestPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Uses /api/profile so the name is decrypted before we store it
        const { data: { user } } = await supabase.auth.getUser();
        if (user) {
          setAdminId(user.id);
          setAdminEmail(user.email ?? null);
          const adminRes = await fetch(`/api/profile?id=${user.id}`);
          if (adminRes.ok) {
            const adminJson = await adminRes.json();
            const p = adminJson.data;
            if (p) {
  ...
  // Every PATCH carries admin identity so the API can write a proper audit row
  ...
  const handleGenerate = async () => {
    ...
    } catch (e: unknown) { setError(e instanceof Error ? e.message : 'Failed to generate document.'); }
  ...
  const uploadFile = async (file: Blob, fileName: string) => {
    ...
    try {
      ...
      try {
        ...
        // Persist tx hash to DB so it survives page reloads
        await fetch(`/api/requests?id=${id}`, {
          method: 'PATCH', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ chain_tx_hash: txHash, ...auditMeta() }),
        });
            ...
            <div className="flex items-end justify-between flex-wrap gap-4">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  {daysAgo >= 2 && request.status === 'pending' && <span className="flex items-center gap-1 mono text-[10px] font-bold text-red-600 dark:text-red-400"><AlertTriangle className="w-3 h-3" />{daysAgo} days waiting</span>}
                </div>
              ...
              <Link href="/pending-requests" className="mono text-[11px] tracking-[0.1em] uppercase text-orange-600 dark:text-orange-400 hover:text-orange-700 transition-colors flex items-center gap-1">← Pending Requests</Link>
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14 }} className="space-y-8">
              <div>
                ...
                {request.status === 'pending' ? (
                  ...
                  <div className="flex items-center gap-2 py-2">
                    {request.status === 'approved' ? <CheckCircle className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> : <XCircle className="w-4 h-4 text-red-600 dark:text-red-400" />}
                    <span className={`text-[13px] font-semibold capitalize ${request.status === 'approved' ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>{request.status}</span>
                  </div>
                )}
                {request.notes && (
                  <div className="mt-4 border-l-2 border-[#c8c6c0] dark:border-[#2a2a32] pl-3">
                    <p className="mono text-[10px] tracking-[0.1em] uppercase text-[#7a7870] dark:text-[#7e7b75] mb-1">{request.status === 'rejected' ? 'Rejection Reason' : 'Approval Notes'}</p>
              ...
              <div>
                ...
                <div className="space-y-2.5">
                  {(error || success || chainError) && (
                    <div className="space-y-2">
                      {chainError   && <AlertBanner variant="warning">{chainError} — <a href="https://metamask.io" target="_blank" className="underline">MetaMask</a> must be connected to Sepolia.</AlertBanner>}
                    </div>
                  )}
                  {chainRecording && (
      ...
      <Modal open={showApproveModal} onClose={() => setShowApproveModal(false)} title="Approve Request">
        <div className="space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button onClick={handleApprove} disabled={processing} className="flex-1 flex items-center justify-center gap-2 py-2.5 text-[12px] font-semibold bg-orange-600 text-white hover:bg-orange-700 transition-colors disabled:opacity-40">
              {processing ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />}{processing ? 'Processing…' : 'Confirm Approval'}
            </button>
          </div>
        </div>
      </Modal>
