...
import {
  ...
  User, Mail, FileText, Clock, ShieldCheck, ExternalLink, Loader2,
} from 'lucide-react';
...
export default function AuditLogDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      // ── Fetch the audit log row + joined request ─────────────────────────
      const { data: logRow, error } = await supabase
        .from('audit_logs')
        .select(`
          id, request_id, action, performed_by, performer_email, notes, created_at,
          requests ( document_type, type, status, file_url, file_hash, user_id )
        `)
        .eq('id', id)
        .single();
      ...
      if (userId) {
        try {
          ...
          if (res.ok) {
            ...
            const p = j.data;
            if (p) {
              residentName  = `${p.firstName ?? p.first_name ?? ''} ${p.lastName ?? p.last_name ?? ''}`.trim();
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}
              ...
              <div>
                ...
                <IconDetail icon={User}  label="Admin ID"    value={log.performed_by ?? 'Unknown'} />
                <IconDetail icon={Mail}  label="Admin Email" value={log.performer_email ?? 'Unknown'} />
              </div>
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14 }}
              ...
              <div>
                ...
                <div className="space-y-3">
                  ...
                  {/* Link to all audit logs for same request */}
                  <div>
                    <p className="mono text-[11px] tracking-[0.1em] uppercase text-[#7a7870] dark:text-[#7e7b75] mb-2">All Events for Request</p>
                    <Link
                      href={`/audit-logs?request_id=${log.request_id}`}
                      className="flex items-center gap-2 mono text-[11px] text-orange-600 dark:text-orange-400 hover:underline"
                    >
                      <ExternalLink className="w-3.5 h-3.5" />
                      View all events →
