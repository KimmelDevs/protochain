...
import { Loader2, CheckCircle, XCircle, Upload, Search, Filter, FileText } from 'lucide-react';
...
const ACTION_CFG = {
  approved: {
    label: 'Approved',
    icon:  CheckCircle,
    cls:   'text-emerald-700 dark:text-emerald-400 border-emerald-300 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-950/30',
  },
  rejected: {
    label: 'Rejected',
    icon:  XCircle,
    cls:   'text-red-600 dark:text-red-400 border-red-300 dark:border-red-700 bg-red-50 dark:bg-red-950/30',
  },
  document_uploaded: {
    label: 'Doc Uploaded',
    icon:  Upload,
    cls:   'text-blue-600 dark:text-blue-400 border-blue-300 dark:border-blue-700 bg-blue-50 dark:bg-blue-950/30',
  },
} as const;
...
export default function AuditLogsPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // ── Fetch audit_logs joined with requests for doc type ───────────────
      const { data: rawLogs, error } = await supabase
        .from('audit_logs')
        .select(`
          id, request_id, action, performed_by, performer_email, notes, created_at,
          requests ( document_type, type, user_id )
        `)
      ...
      // ── Collect unique user_ids to resolve resident names ────────────────
      const userIds = [...new Set(
        (rawLogs ?? []).map((l: any) => l.requests?.user_id).filter(Boolean)
      )];

      // Fetch profiles via API so they are decrypted
      ...
      const shaped: AuditLog[] = (rawLogs ?? []).map((l: any) => ({
        id:              l.id,
  ...
  return (
    <>
      <style>{`
        ...
        .pg   { font-family: 'IBM Plex Sans', monospace; }
            ...
            {filtered.length === 0 ? (
              <div className="border border-[#c8c6c0] dark:border-[#2a2a32] bg-white dark:bg-[#1e1e24] py-16 text-center">
                ...
                <p className="text-[13px] text-[#7a7870] dark:text-[#7e7b75]">
                  {logs.length === 0 ? 'No audit events yet. Events are recorded when admins approve, reject, or upload documents.' : 'No events match your filters.'}
                </p>
              </div>
            ) : (
              <div className="border border-[#c8c6c0] dark:border-[#2a2a32] bg-white dark:bg-[#1e1e24] overflow-x-auto">
                ...
                {/* Header */}
                <div className="grid grid-cols-[1fr_1.4fr_1.2fr_1fr_1.2fr_0.8fr] gap-4 px-5 py-3 border-b border-[#e8e5e0] dark:border-[#222228] bg-[#f5f4f0] dark:bg-[#16161a]">
                  {['Timestamp', 'Admin', 'Action', 'Document Type', 'Resident', 'Request'].map(h => (
                ...
                {filtered.map((log, i) => (
                  <div
                    key={log.id}
                    className={`grid grid-cols-[1fr_1.4fr_1.2fr_1fr_1.2fr_0.8fr] gap-4 px-5 py-4 border-b border-[#e8e5e0] dark:border-[#222228] last:border-0 transition-colors hover:bg-[#f5f4f0] dark:hover:bg-[#16161a] ${i % 2 === 0 ? '' : 'bg-[#faf9f7] dark:bg-[#1a1a20]'}`}
                  >
                    {/* Timestamp */}
                    <div>
                    ...
                    {/* Admin */}
                    <div>
                      <p className="text-[12px] text-[#1a1917] dark:text-[#f0eee8] truncate" title={log.performer_email ?? ''}>
                        {log.performer_email ?? <span className="text-[#7a7870] dark:text-[#7e7b75] italic">Unknown</span>}
                      </p>
                    </div>
                    ...
                    <div>
                      <Link
                        href={`/approved-documents/${log.request_id}`}
                        className="mono text-[10px] tracking-[0.08em] uppercase text-orange-600 dark:text-orange-400 hover:underline"
                      >
                        {log.request_id.slice(0, 8).toUpperCase()} →
                      </Link>
                    </div>
                  </div>
                ))}
