    ...
    <div className="min-h-screen p-4 lg:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">Rejected Requests</h1>
          <p className="text-gray-400">View all rejected document requests</p>
        </motion.div>
        ...
        >
          {[
            { label: 'Total Rejected', value: requests.length, color: 'text-white' },
            { label: 'This Week', value: requests.filter(r => isThisWeek(r.created_at)).length, color: 'text-red-400' },
            { label: 'This Month', value: requests.filter(r => isThisMonth(r.created_at)).length, color: 'text-orange-400' },
            { label: 'Document Types', value: new Set(requests.map(r => r.type)).size, color: 'text-purple-400' },
          ].map(s => (
            ...
            <CardContent>
              {requests.length === 0 ? (
                <div className="text-center py-16">
                  ...
                  <p className="text-white font-medium mb-1">No rejected requests</p>
                  <p className="text-gray-400 text-sm">Rejected requests will appear here.</p>
                </div>
              ) : (
                <Table>
                  <TableHeader>
