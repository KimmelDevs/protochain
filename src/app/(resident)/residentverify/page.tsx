  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <motion.div
        ...
        className="bg-white dark:bg-[#1e1e24] border border-[#c8c6c0] dark:border-[#2a2a32] w-full max-w-lg shadow-2xl"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3 border-b border-[#c8c6c0] dark:border-[#2a2a32]">
          <div className="flex items-center gap-2">
            <Camera className="w-4 h-4 text-[#0d74ce]" />
            <span className="mono text-[11px] font-bold tracking-[0.15em] uppercase text-[#1a1917] dark:text-[#f0eee8]">
              Webcam Scanner
            </span>
          </div>
          <button onClick={onClose} className="text-[#7a7870] hover:text-[#1a1917] dark:hover:text-[#f0eee8] transition-colors p-1">
            <X className="w-4 h-4" />
          </button>
        </div>

        {/* Mode tabs */}
        <div className="flex border-b border-[#c8c6c0] dark:border-[#2a2a32]">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 mono text-[10px] font-bold tracking-[0.12em] uppercase px-5 py-2.5 transition-colors disabled:opacity-40 ${
                scanMode === m
                  ...
                  : 'text-[#7a7870] dark:text-[#7e7b75] hover:text-[#1a1917] dark:hover:text-[#f0eee8]'
              }`}
            >
              {m === 'qr'
          ...
          <div className="flex-1 flex items-center justify-end pr-3">
            <span className="mono text-[9px] text-[#7a7870] dark:text-[#7e7b75]">
          ...
          {scanMode === 'qr' && !detected && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="w-44 h-44 relative">
                <div className="absolute top-0 left-0 w-6 h-6 border-t-[3px] border-l-[3px] border-[#0d74ce]" />
                <div className="absolute top-0 right-0 w-6 h-6 border-t-[3px] border-r-[3px] border-[#0d74ce]" />
                <div className="absolute bottom-0 left-0 w-6 h-6 border-b-[3px] border-l-[3px] border-[#0d74ce]" />
                <div className="absolute bottom-0 right-0 w-6 h-6 border-b-[3px] border-r-[3px] border-[#0d74ce]" />
                <div className="absolute inset-2 border border-[#0d74ce]/20 animate-pulse" />
              </div>
            </div>
          )}
          ...
          {scanMode === 'ocr' && !scanning && !detected && (
            <div className="absolute bottom-3 inset-x-0 flex justify-center pointer-events-none">
              <div className="flex items-center gap-1.5 bg-black/65 rounded-full px-3 py-1.5">
                ...
                <span className="mono text-[9px] text-blue-200">Zoom in on the 64-char hash text</span>
              </div>
            </div>
          )}
        ...
        {/* Status */}
        <div className="px-5 py-3 border-t border-[#c8c6c0] dark:border-[#2a2a32]">
          <div className="flex items-center gap-2 mb-2">
            {scanning ? (
              <Loader2 className="w-3 h-3 text-[#0d74ce] animate-spin shrink-0" />
            ) : detected ? (
              <Check className="w-3 h-3 text-emerald-500 shrink-0" />
            ) : (
              <div className="w-2 h-2 rounded-full bg-[#0d74ce] animate-pulse shrink-0" />
            )}
            <p className="mono text-[10px] text-[#3d3b36] dark:text-[#c9c6be]">{status}</p>
          </div>
          {scanning && ocrProgress > 0 && (
            <div className="w-full bg-[#c8c6c0] dark:bg-[#2a2a32] h-1 overflow-hidden">
              <div
                className="h-full bg-[#0d74ce] transition-all duration-300"
        ...
        {scanMode === 'ocr' && (
          <div className="px-5 pb-4">
            <button
              ...
              className="flex items-center justify-center gap-2 w-full bg-[#E8500A] hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed text-white mono text-[11px] font-bold tracking-[0.08em] uppercase py-2.5 transition-colors"
            >
              {scanning
                ...
                <motion.div key="scan" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-5">
                    <button
                      ...
                      className="group flex flex-col items-center gap-3 border-2 border-dashed border-[#c8c6c0] dark:border-[#2a2a32] hover:border-[#0d74ce] p-6 transition-colors"
                    >
                      <div className="w-10 h-10 bg-[#0d74ce]/10 group-hover:bg-[#0d74ce]/20 flex items-center justify-center transition-colors">
                        <QrCode className="w-5 h-5 text-[#0d74ce]" />
                      </div>
                      <div className="text-center">
                        <p className="mono text-[11px] font-bold tracking-[0.1em] uppercase text-[#1a1917] dark:text-[#f0eee8] mb-1">QR Code</p>
                        <p className="mono text-[9px] text-[#7a7870] dark:text-[#7e7b75] leading-relaxed">Auto-detects & reads QR codes continuously</p>
                      </div>
                    </button>
                    <button
                      onClick={() => setShowWebcam(true)}
                    ...
                    >
                      <div className="w-10 h-10 bg-[#0d74ce]/10 group-hover:bg-[#0d74ce]/20 flex items-center justify-center transition-colors">
                        <ScanText className="w-5 h-5 text-[#0d74ce]" />
                      </div>
                      <div className="text-center">
                        <p className="mono text-[11px] font-bold tracking-[0.1em] uppercase text-[#1a1917] dark:text-[#f0eee8] mb-1">OCR Text</p>
                        <p className="mono text-[9px] text-[#7a7870] dark:text-[#7e7b75] leading-relaxed">Captures a frame and extracts the printed hash</p>
                      </div>
                    </button>
                  </div>
                  {hash && (
              ...
              >
                {isAuthentic && (
                  <>
                    ...
                    <a href={`https://sepolia.etherscan.io/address/${result.recordedBy}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mono text-[11px] font-bold tracking-[0.1em] uppercase text-emerald-700 dark:text-emerald-400 hover:underline">
